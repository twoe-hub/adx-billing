...
The `home-routes` handler in the `adx-billing.routes.home` namespace
...
The `home-page` function will in turn call the `adx-billing.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `adx-billing.handler` namespace:
...
Request middleware functions are located under the `adx-billing.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `adx-billing.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `adx-billing.db.core` in some other namespace.
