...
(defonce url "/inv/invs")
(defonce cols ['id 'no 'inv-no 'wo-no 'quote-no 'value 'cat 'sub-cat 'issued-to 'issued-by 'date-issued])
(defonce no-sort-cols ['no])
(defonce params (rcore/atom {:sort "date-issued" :order 1 :offset 0 :limit ls/pg-size}))
(defonce invs (rcore/atom {}))
...
(defn save-inv [fields errors invs]
     ...
     [:div.modal-card
      ...
      [:footer.modal-card-foot.is-right.pt-50.pb-20.pr-20
       ...
       [:button.button {:on-click #(toggle-modal modal-id fields errors)}
        "Cancel"]
       [:button.button {:on-click #(save-inv fields errors invs)}
        "Save"]]]]))
   ...
   [:ul
    [:li {:key "all"}
     [:a {:on-click #((swap! params dissoc :status)
      ...
      (str (msg (keyword (str "inv.qf-label/" "all"))) ": " (@counts :all 0))]]
    [:li {:key "draft"}
     [:a {:on-click #((swap! params assoc :status "DRAFT")
                      (ls/get-records url params handler))}
      (str (msg (keyword (str "inv.qf-label/" "draft"))) ": " (@counts :draft 0))]]
    [:li {:key "sent"}
     [:a {:on-click #((swap! params assoc :status "SENT")
                      (ls/get-records url params handler))}
      (str (msg (keyword (str "inv.qf-label/" "sent"))) ": " (@counts :sent 0))]]
    [:li {:key "approved"}
     [:a {:on-click #((swap! params assoc :status "APPROVED")
                      (ls/get-records url params handler))}
      (str (msg (keyword (str "inv.qf-label/" "approved"))) ": " (@counts :approved 0))]]
    [:li {:key "completed"}
     [:a {:on-click #((swap! params assoc :status "COMPLETED")
                      (ls/get-records url params handler))}
      (str (msg (keyword (str "inv.qf-label/" "completed"))) ": " (@counts :completed 0))]]
    ]])
...
(defn table-filter-row []
  [:tr#listing-filter.is-hidden
   ...
   [:th>div.field
    [:div.control
     [input-el 'inv-no 'inv-no 'text "input" "" "inv-no"]]]
   [:th>div.field
    [:div.control
     [input-el 'wo-no 'wo-no 'text "input" "" "wo-no"]]]
   [:th>div.field
    [:div.control
...
(defn table-ui []
  [:table.listing-table.table.is-fullwidth.is-striped.is-hoverable
   [:thead
    [ls/table-head-row "inv" cols no-sort-cols url params handler]
    (table-filter-row)]
   [:tbody.listing-content
    (for [{:keys [id no inv-no wo-no quote-no issued-to value issued-by date-issued cat sub-cat]} @invs]
      ^{:key id}
      [:tr {:style {:border "none"}}
       ...
       [:td {:style {:border "none"}} inv-no]
       [:td {:style {:border "none"}} wo-no]
